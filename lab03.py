    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == MOUSEBUTTONUP:
                ...
                elif top ==2 and side ==9 and solution == None:
                    solution = 'A STAR'
                    sol = copy.deepcopy(initial)
                    print("A STAR")
                    astar()
                elif top ==2 and side ==9 and solution == 'A STAR':
                    if len(path) > 0:
                        next()
                    else : 
                        board = sol
    ...
    for x in range(3):
        ...
        if x == 2:
            if text == 'A STAR':
                ...
                textSurf = BASICFONT.render('A STAR', True, BLACK)
            textRect = textSurf.get_rect()
            textRect.center = 600 + 30 , (SCREEN_HEIGHT/6) * i - 10 + 20,
            screen.blit(textSurf, textRect)
...
def createBoard(b):
    fileSurf = BASICFONT.render('Press SPACE to select new file input', True, BLACK)
    fileRect = fileSurf.get_rect()
    fileRect.center = SCREEN_WIDTH/2, 50
...
def DFS():
    ...
    while len(frontier) > 0: 
        ...
        if b== [[1,2,3],[4,5,6],[7,8,0]]:
            ...
            print('======================================')
            print(explored.index(b))
            print(explored[explored.index(b)])
            continue
...
def astar():
    ...
    openList.append(copy.deepcopy(current))
    while len(openList) > 0:
        bestNode = min(x[3] for x in openList)
        for z in range(len(openList)):
            if openList[z][3]  == bestNode:
                bestNode = openList.pop(z)
                break
        b = bestNode[:-2]
        f = bestNode[3]
        direction = bestNode[4]
        if b== [[1,2,3],[4,5,6],[7,8,0]]:
            closedList.append(copy.deepcopy(b))
            p = direction
            break
        elif b in closedList:
            continue
        else:
            closedList.append(copy.deepcopy(b))
            blankx, blanky = blank(b)
            if blankx-1 >=0:
                #get next board when moving
                temp = copy.deepcopy(b)
                move(b,'up')
                # get direction
                tempd = direction
                ...
                # get f
                tempf = f
                f = len(direction) - direction.count(' ') + manhattan(b)
                b.append(f)
                b.append(direction)
                openList.append(b)
                b = copy.deepcopy(temp)
